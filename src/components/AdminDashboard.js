...
const AdminDashboard = () => {
  ...
  const fetchStudents = async () => {
    try {
      const response = await axios.get('http://13.60.234.251:8000/get_students', {
        headers: {
  ...
  const handleAddStudent = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://13.60.234.251:8000/add_student', {
        username,
        password,
      }, {
        headers: {
  ...
  const handleAddCourse = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://13.60.234.251:8000/add_course', {
        name: courseName,
      }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
